...
const inputTitulo = document.querySelector(
...
document.querySelectorAll('.day_week div').forEach((div) => {
    div.addEventListener('click', () => {
        document
            .querySelectorAll('.day_week div')
            .forEach((d) => d.classList.remove('active'));
        div.classList.add('active');
        inputTitulo.value = div.dataset.day;
    });
});
...
btnConcluir.addEventListener('click', async () => {
    const titulo = document
        .querySelector('div.rowtitle input.new_exercice')
        .value.trim();
    if (!titulo) {
    ...
    document.querySelectorAll('form.rowinput .rowtext').forEach((row) => {
        ...
        const repeticoes =
            parseInt(row.querySelector('input.reps')?.value) || 0;
